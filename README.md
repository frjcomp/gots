...
**Quick tips:**
First connection without a fingerprint will still work with a self-signed cert, but the listener `gotsl` logs a warning and prints the certificate fingerprint. Copy it and reconnect with `--cert-fingerprint` for security.
...
- Self-signed without fingerprint: The connection is allowed, but the client logs a clear security warning and prints the server fingerprint so you can reconnect with `--cert-fingerprint <hash>`.
