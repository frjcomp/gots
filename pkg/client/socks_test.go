...
func (m *mockResolver) LookupAddr(ctx context.Context, addr string) ([]string, error) {
	...
	if res, ok := m.reverse[addr]; ok {
		return res, nil
	}
	return nil, nil
}

func TestResolveDialAddressesPrefersIPv4(t *testing.T) {
	...
	addrs := resolveDialAddresses("dualstack.test", "8080")
	if len(addrs) == 0 {
		t.Fatal("expected at least one address")
	}
	if !strings.HasPrefix(addrs[0], "127.0.0.1:") {
		t.Fatalf("expected IPv4 first, got %s", addrs[0])
	}
}

func TestResolveDialAddressesIPv6FallbackToReverseIPv4(t *testing.T) {
	orig := defaultResolver
	defer func() { defaultResolver = orig }()
	...
	addrs := resolveDialAddresses(v6, "443")
	if len(addrs) < 2 {
		t.Fatalf("expected fallback and original addresses, got %v", addrs)
	}
	if addrs[0] != "203.0.113.10:443" {
		t.Fatalf("expected IPv4 fallback first, got %s", addrs[0])
	}
	if addrs[1] != "[2001:db8::1]:443" {
		t.Fatalf("expected original IPv6 preserved, got %s", addrs[1])
	}
}
